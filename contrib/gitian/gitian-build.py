...
gsigs = 'https://github.com/idlechain-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('idlechain'):
        # Remove the potentially stale idlechain dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'idlechain', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'idlechain'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/idlechain/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'idlechain='+args.commit, '--url', 'idlechain='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/idlechain-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/idlechain/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/idlechain/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/idlechain-project/idlechain', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/idlechain', exist_ok=True)
    os.chdir('builder/inputs/idlechain')
    if args.pull:
