...
Core tests take longer than any other IDLEChain Project tests, due to the high amount of computational work involved in validating core components.
...
To run only IDLEChain Project's core tests (after building):
...
To run only IDLEChain Project's crypto tests (after building):
...
```bash
idlechaind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `idlechain-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only IDLEChain Project's hash tests (after building):
...
To run only IDLEChain Project's performance tests (after building):
...
To run only IDLEChain Project's unit tests (after building):
