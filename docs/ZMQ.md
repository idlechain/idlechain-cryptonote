# The Current/Future Status of ZMQ in IDLEChain Project

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. IDLEChain Project
...
matching supported by both IDLEChain Project and ZMQ. The `format`, `context` and `event`
...
The IDLEChain Project daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The IDLEChain Project daemon also ensures that
...
The IDLEChain Project daemon will send a `txpool_add` pub exactly once for each
